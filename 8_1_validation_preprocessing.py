"""
Preprocessing della validazione finale SEVIRI.

Questo modulo raccoglie le fasi preliminari necessarie per la validazione:
- conversione dei file nativi SEVIRI in NetCDF ritagliati sull'area di studio;
- generazione delle mappe diagnostiche del canale 9;
- creazione delle maschere stagionali, terra/mare e giorno/notte;
- esportazione dei canali e delle maschere in formato pickle.

Il comportamento operativo resta quello del file originale di validazione, ma le parti
di preprocessing sono isolate per migliorare leggibilità e manutenzione.
"""
...
def compute_latlon_actual(scn: Scene):
    """Calcola le coordinate geografiche reali della scena SEVIRI a partire dalla proiezione geostazionaria."""
    vis006 = scn["VIS006"]
...
def nat2net_crop_actual(source_dir, zip_name, out_dir, lat_min, lat_max, lon_min, lon_max, bbox=None):
    """Converte file ZIP nativi SEVIRI in NetCDF e applica il ritaglio geografico."""
    import zipfile
    ...
    if bbox is None:
        ...
        if len(xs) == 0 or len(ys) == 0:
            print(f"Nessun pixel nel box per {zip_name}")
            os.remove(nat_path)
            return None, None
...
def batch_convert_crop_actual(source_dir, out_dir, lat_min=43.4, lat_max=45.8, lon_min=9.15, lon_max=13.2):
    """Converte in sequenza tutti i file ZIP nativi presenti nella cartella sorgente."""
    ensure_dir(out_dir)
    bbox = None
...
def generate_ch9_maps(input_dir, output_dir):
    """Genera mappe geospaziali del canale 9 dai file NetCDF preprocessati."""
    ...
    for file_name in file_list:
        ...
        try:
            ...
            print(f"Errore nella generazione mappa CH9 per {file_name}: {error}")
...
def generate_season_mask(nc_dir, mask_dir):
    """Genera la maschera stagionale per tutti i file NetCDF."""
    ensure_dir(mask_dir)
    season_mask = []
...
def load_sealand_mask(netcdf_file):
    """Carica la maschera terra/mare globale da file NetCDF."""
...
def generate_sealand_masks(nc_dir, mask_dir, sealand_file):
    """Rigenera le maschere terra/mare sulle coordinate dei dati SEVIRI preprocessati."""
    ensure_dir(mask_dir)
    lat_list = []
    lon_list = []
...
def generate_day_night_masks(nc_dir, mask_dir, day_threshold=70.0):
    """Classifica ogni scena come diurna o notturna in base alla percentuale di pixel illuminati."""
    ensure_dir(mask_dir)
    day_mask = []
    night_mask = []
...
def export_pickles_from_nc_and_masks(nc_dir, mask_dir, out_dir):
    """Esporta canali NetCDF, coordinate e maschere in file pickle allineati alla pipeline ML."""
    ensure_dir(out_dir)
