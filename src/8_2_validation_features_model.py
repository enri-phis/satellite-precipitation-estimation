"""
Feature construction and model inference for final validation.

This module isolates the most numerically dense part of the validation pipeline:
- channel cleaning and NaN replacement;
- construction of local and geographic features;
- block-wise saving to contain memory usage;
- training/application of the Random Forest model on validation blocks.
"""
...
def replace_nans_with_nearest(arr):
    """Replaces NaNs with the nearest valid value along the vector."""
    arr = arr.copy()
    n = len(arr)
    isnan = np.isnan(arr)
    notnan = ~isnan
...
def replace_nans_with_nearest_blockwise(arr, block_size=1_232_000):
    """Applies NaN replacement in blocks to reduce memory load."""
...
def moving_mean_std_images(vector_1d, width, height, kernel_size=5):
    """Computes local mean and standard deviation for each image in the stack."""
...
def build_features(processed_dir, output_dir):
    """Builds feature blocks used in the validation stage."""
...
def run_model(features_dir, balanced_features_file, balanced_dpr_file, balanced_day_mask_file, model_out_dir):
    """Trains the RF model on balanced data and generates predictions on validation blocks."""
