"""
Aggregation of precipitation classes from 15 to 7 and then to 5 classes.

The script:
- loads true and predicted labels from the 15-class model,
- applies deterministic mappings 15 -> 7 -> 5,
- recomputes classification metrics on aggregated groups,
- saves grouped labels, text reports, and normalized confusion matrices.
"""
...
def salva_risultati_grouping(y_true_lbl: np.ndarray, y_pred_lbl: np.ndarray, class_names: List[str], output_dir: str, suffix: str) -> None:
    ...
    txt = (
        ...
        f"Empirical HSS: {metrics['hss_empirical']:.4f}\n"
...
def esegui_grouping_15_7_5(output_dir_15: str) -> None:
    ...
    print("Grouping 15 -> 7 -> 5 completed and saved.")
