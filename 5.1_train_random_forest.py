"""
Addestramento Random Forest per classificazione multi-classe della precipitazione.

Il programma:
- carica feature e target DPR dai dataset ML,
- filtra i campioni in condizioni diurne,
- addestra un classificatore Random Forest per il numero di classi selezionato,
- calcola metriche (Accuracy, Precision, Recall, F1, HSS),
- salva modelli, label predette/reali e figure diagnostiche (confusion matrix, ROC, importanza feature).
"""
...
def train_rf_daytime(ml_dir: str, output_dir: str, class_key: str, rf_params: Optional[Dict] = None) -> None:
    """Addestra Random Forest usando solo pixel diurni e salva metriche/artefatti."""
    ...
    plt.title(f"Confusion Matrix - RF - giorno ({class_key} classi)")
    plt.tight_layout()
    ...
    plt.title(f"ROC - RF - giorno ({class_key} classi)")
    ...
    plt.ylabel("Importanza feature")
    plt.title(f"Ranking feature RF ({class_key} classi)")
    plt.tight_layout()
    ...
    print(f"Training RF completato ({class_key} classi).")
